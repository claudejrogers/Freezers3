...
def _remodel_cells(fid, cur, new_box_capacity, new_box_width, faddress,
...
@transaction.autocommit
def _make_default_box(f, sid, rid, did, bid):
    """
...
@transaction.autocommit
def _make_default_box_1(f, cur, base_addr, bid):
    """
    Make a new, empty box in given location.
    """
...
@transaction.autocommit
def _remodel_boxes(fid, cur, new_drawer_capacity, base_address, box_ids,
    ...
    if new_drawer_capacity < cur_max_box:
        ...
        for bid in xrange(cur_max_box + 1, new_drawer_capacity + 1):
            _make_default_box_1(f, cur, base_address, bid)
        for bid in box_ids:
...
@transaction.autocommit
def _remodel_drawers(fid, cur, new_rack_capacity, base_address, drawer_ids,
    ...
    if new_rack_capacity < cur_max_drawers:
        ...
        if cur_max_address > org_max_address:
            cur.execute(
                """SELECT ID FROM freezers_samplelocation WHERE
                   freezer_id = %s AND address >= %s AND address < %s AND
                   occupied = True""",
                (fid, new_max_address, laddress)
            )
            occupied_sl = cur.fetchall()
        ...
        for did in xrange(cur_max_drawers + 1, new_rack_capacity + 1):
            ...
            for bid in xrange(1, f.box_capacity + 1):
                _make_default_box_1(f, cur, baddr, bid)
        for did in drawer_ids:
            ...
            if not this_drawer:
                for bid in xrange(1, f.box_capacity + 1):
                    fbaddress = ((base_address << 24) + (did << 16) +
                                 (bid << 8) + 1)
                    lbaddress = fbaddress + 0x0100
                    cur.execute("""UPDATE freezers_samplelocation
                                       SET occupied = False,
...
def remodel_cells_in_freezer_subsection(new_value, box_dim, fid, sid,
    ...
    for k, v in locdict.iteritems():
        ...
        cur_max_address = v
        _remodel_cells(fid, cur, new_box_capacity, new_box_width, faddress,
                       laddress, cur_max_address, new_max_address,
                       org_max_address)
...
def remodel_boxes_in_freezer_subsection(new_value, fid, sid, rid=None,
    ...
    for k, v in locdict.iteritems():
        ...
        _remodel_boxes(fid, cur, new_drawer_capacity, base_address, box_ids,
                       laddress, new_max_address, org_max_address,
                       cur_max_address)
...
def remodel_drawers_in_freezer_subsection(new_value, fid, sid, rid=None):
    ...
    for k, v in locdict.iteritems():
        ...
        _remodel_drawers(fid, cur, new_rack_capacity, base_address, drawer_ids,
                         faddress, laddress, new_max_address, org_max_address,
                         cur_max_address)
...
def remodel_racks_in_freezer_subsection(new_value, fid, sid):
    ...
    if new_shelf_capacity < cur_max_racks:
        ...
        cur_max_address = max([address for (address,) in cur.fetchall()])
        cur.execute("""SELECT ID FROM freezers_samplelocation WHERE
                       freezer_id = %s AND address >= %s AND address < %s AND
                       occupied = True""", (fid, new_max_address, laddress))
        occupied_sl = cur.fetchall()
        if occupied_sl:
        ...
        for rid in xrange(cur_max_racks + 1, new_shelf_capacity + 1):
            for did in xrange(1, f.drawer_capacity + 1):
                for bid in xrange(1, f.box_capacity + 1):
                    _make_default_box(f, sid, rid, did, bid)
        for rid in rack_ids:
